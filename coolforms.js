...
(function() {
  ...
  angular.module('CoolForm').directive('coolform', function(definitionService, registrationService) {
    var l;
    l = function(scope, elem, attr) {
      ...
      return scope.$watch('form', function(v) {
        if ((v != null) && (v.form != null)) {
          ...
          return scope.service.registerController(scope, scope.definition);
        }
      });
    };
    return {
  ...
  angular.module('CoolFormDirectives').directive('coolformField', function() {
    ...
    l = function(scope) {
      scope.error = false;
      return scope.service.watchFieldValidation(scope.field.name, function() {
  ...
  angular.module('CoolFormDirectives').directive('coolformSubmit', function() {
    var l;
    l = function(scope) {
      return scope.submit = function() {
        return scope.service.validateAllFields();
      };
    };
    return {
      restrict: 'E',
  ...
  angular.module('CoolFormDirectives').directive('coolformWizard', function() {
    var l;
    l = function(scope) {
      var field, line, p, page, pageFields, validatePage, watchField, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _ref2, _results;
      scope.current = 0;
      ...
      watchField = function(page, fieldName) {
        return scope.service.watchFieldValidation(fieldName, function() {
  ...
  angular.module('CoolFormServices').factory('registrationService', function(validators) {
    return function() {
      var addHandler, errorHandlers, errors, fields, okHandlers, registerController, registerField, removeError, sendError, sendOk, services, setFields, validateAllFields, validateField, validateFields, valueChange, values, watchFieldValidation;
      fields = {};
      errors = {};
      values = {};
      errorHandlers = {};
      ...
      setFields = function(pages) {
        var field, line, p, page, _i, _j, _k, _len, _len1, _len2, _ref, _ref1, _results;
        p = 0;
        _results = [];
      ...
      sendError = function(fieldName, error) {
        var handler, _i, _len, _ref, _results;
        if (errorHandlers[fieldName] == null) {
          return;
        }
        _ref = errorHandlers[fieldName];
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          handler = _ref[_i];
          _results.push(handler(error));
        }
        return _results;
      };
      sendOk = function(fieldName) {
        var handler, _i, _len, _ref, _results;
        if (okHandlers[fieldName] == null) {
          return;
        }
        _ref = okHandlers[fieldName];
        _results = [];
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          handler = _ref[_i];
        ...
        if (field.validation != null) {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            ...
            if (validators[rule.validator] != null) {
              if (validators[rule.validator](fieldName, values, rule) === false) {
      ...
      validateAllFields = function() {
        return validateFields(Object.keys(fields));
      };
      valueChange = function(fieldName, value) {
        values[fieldName] = value;
        return removeError(fieldName);
      };
      addHandler = function(field, fn, list) {
        if (list[field] == null) {
          list[field] = [];
        }
        if (fn != null) {
          return list[field].push(fn);
        }
      };
      registerController = function(controllerScope, form) {
        return setFields(form.pages);
      };
      watchFieldValidation = function(fieldName, onOk, onError) {
        if (onOk != null) {
          addHandler(fieldName, onOk, okHandlers);
        }
        if (onError != null) {
          return addHandler(fieldName, onError, errorHandlers);
        }
      };
      registerField = function(fieldName, onOk, onError) {
        watchFieldValidation(fieldName, onOk, onError);
        return function(value) {
          return valueChange(fieldName, value);
        };
      };
      services = {
